...
class EdgeQuantumSimulator(TieredSimulator):
    ...
    def __init__(self, n_qubits):
        # Increased fusion_threshold to 60 for better I/O amortization
        super().__init__(n_qubits, use_compression=True, fusion_threshold=60, use_managed_memory=True)
...
def run_single_benchmark(sim_class, sim_name, n_qubits, circuit_fn, circuit_name, timeout_sec=600, **kwargs):
    ...
    try:
        with timeout(timeout_sec):
            # Create simulator
            print(f"[DEBUG] Creating simulator {sim_name}...", file=sys.stderr)
            sim = sim_class(n_qubits)
            
            # Initialize
            print(f"[DEBUG] Initializing...", file=sys.stderr)
            t0 = time.time()
            sim.init_zero_state()
            init_time = time.time() - t0
            print(f"[DEBUG] Init done in {init_time:.4f}s", file=sys.stderr)
            
            # Run circuit
            print(f"[DEBUG] Running circuit...", file=sys.stderr)
            t0 = time.time()
            gate_count = circuit_fn(sim, **kwargs) if kwargs else circuit_fn(sim)
            circuit_time = time.time() - t0
            print(f"[DEBUG] Circuit done in {circuit_time:.4f}s", file=sys.stderr)
            ...
            sim.cleanup()
            print(f"[DEBUG] Cleanup done", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    if args.run_single:
        ...
        try:
            ...
            # Define simulators (Order matters: Run stable/important ones first)
            sim_map = {
