...
class BMQSimBaseline(TieredSimulator):
    ...
    """
    def __init__(self, n_qubits):
        # Use managed memory for stability (like EdgeQuantum but without async prefetch)
...
def run_benchmark_suite():
    ...
    # Simulators to test - PHASE 2: HIGH QUBIT (GPU-ONLY)
    # CPU sims removed due to RAM limit and hang issues at 28Q+
    simulators = [
        (CuQuantumNative, "cuQuantum (Native)"),
        (CuQuantumUVM, "cuQuantum (UVM)"),
        (BMQSimBaseline, "BMQSim-like (Swap)"),
        (TieredSimulator, "EdgeQuantum (Ours)"),
    ]
    
    # Test configurations - PHASE 2: 28-37 qubits (GPU-only)
    qubit_counts = [28, 30, 32, 34, 36, 37]
