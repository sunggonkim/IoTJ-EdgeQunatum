...
\textbf{Software}: Ubuntu 20.04 (JetPack 5.1), CUDA 11.4 (nvcc 11.4, C++17), cuQuantum SDK (ARM64/SBSA archive) v22.03 with cuStateVec v1.0, Python 3.8, and LZ4 (liblz4 1.9.x). We use the SBSA archive because it provides CUDA 11.x-compatible ARM64 binaries for Jetson-class devices; newer SBSA archives (23.10+) only ship PTX for \texttt{sm\_80/90}, while the 22.03 archive includes \texttt{sm\_86} PTX that successfully JITs on Orin (SM 8.7). For reproducibility, the build sets \texttt{CUQUANTUM\_ROOT} to the extracted archive and links with an rpath to \texttt{\$\{CUQUANTUM\_ROOT\}/lib}; the simulator is compiled with \texttt{-O3 -arch=sm\_87 -std=c++17}. Python benchmarks use \texttt{numpy} and \texttt{cirq} (pip installed) and invoke the same circuit families as the C++ runner.

\textbf{Schemes (all on the same device)}:
\begin{itemize}
    \item \textbf{cuQuantum Native}: GPU-resident state vector via cuStateVec; all chunks fit in GPU memory.
    \item \textbf{cuQuantum UVM}: Unified Virtual Memory using \texttt{cudaMallocManaged} with prefetch to GPU.
    \item \textbf{BMQSim-like Offload}: Blocking tiered I/O without asynchronous prefetch (host-managed paging).
    \item \textbf{Cirq}: CPU state-vector simulation using Python (baseline correctness and timing).
    \item \textbf{EdgeQuantum}: Tiered NVMe/DRAM/GPU pipeline with asynchronous prefetch and LZ4 compression.
\end{itemize}
